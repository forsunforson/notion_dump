...
class TestIndexGeneratorService(unittest.IsolatedAsyncioTestCase):
    async def test_rebuild_all_rewrites_index_from_all_markdown_files(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            try:
                ...
                (output_dir / "index.json").write_text('{"stale.md": {"summary": "old"}}\n', encoding="utf-8")

                service = IndexGeneratorService(use_llm=False)
                rebuilt = await service.rebuild_all()

                self.assertEqual(set(rebuilt.keys()), {"a.md", "b.md"})
                index_data = json.loads((output_dir / "index.json").read_text(encoding="utf-8"))
                self.assertEqual(set(index_data.keys()), {"a.md", "b.md"})
            finally:
