    ...
    async def handle_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            system_prompt = (
                ...
                "【🚨 核心工具调用原则 - 绝对遵守】\n"
    "1. 当用户提供或修改任何健康数据（如体重、精力、睡眠、训练打分等）时，你**必须且只能**通过调用 `upsert_daily_metric` 工具来记录！\n"
    "2. **绝对禁止口头答应！**在没有真正调用工具之前，绝不允许回复“收到”、“已记录”等敷衍文字。\n"
    "3. 只有当你调用了工具，并且收到了工具执行成功的返回信息后，你才能用自然语言向用户汇报结果。\n\n"
                "请用简短、友好的风格回复用户的消息。"
            )
